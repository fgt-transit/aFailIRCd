...
DECLARE_MODULE_AV1(mkpasswd, NULL, NULL, mkpasswd_clist, NULL, NULL, "SporksNet coding committee");
...
m_mkpasswd(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	if(!irccmp(hashtype, "BLOWFISH"))
		salt = make_bf_salt(6, 22);
	else if(!irccmp(hashtype, "SHA256"))
	...
	else
	{
		sendto_one_notice(source_p,
				  ":MKPASSWD syntax error:  MKPASSWD pass [BLOWFISH|SHA256|SHA512|MD5]");
		return 0;
	}
