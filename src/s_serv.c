 ...
 * output	-
 * side effects - client is sent a list of +b/+e/+I/+d modes
 *
 * XXX:
 * This should probably be made more extensible... and we shouldn't use bmask
 * for just any list. --Elizabeth
 */
static void
...
burst_TS6(struct Client *client_p)
{
	...
	RB_DLINK_FOREACH(ptr, global_channel_list.head)
	{
		...
		if(rb_dlink_list_length(&chptr->quietlist) > 0)
			burst_modes_TS6(client_p, chptr, &chptr->quietlist, 'Z');
		
		if(rb_dlink_list_length(&chptr->censorlist) > 0)
			burst_modes_TS6(client_p, chptr, &chptr->censorlist, 'y');
