...
construct_cflags_strings(void)
{
	...
	*ptr2 = '\0';
	...
	for(i = 0; i < 256; i++)
	{
		if( !(chmode_table[i].set_func == chm_list) &&
			...
			!(chmode_table[i].set_func == chm_founder) &&
		...
		if( !(chmode_table[i].set_func == chm_nosuch) && 
			...
			!(chmode_table[i].set_func == chm_founder && !ConfigChannel.use_founder) &&
...
construct_cflag_param_string(void)
{
	...
	*cflagsparaminfo = '\0';
	rb_snprintf(cflagsparaminfo, sizeof cflagsparaminfo, "%sb%s%s%s%s%sklov%s%s",
			ConfigChannel.use_founder ? "q" : "",
			ConfigChannel.use_admin ? "a" : "",
			ConfigChannel.use_except ? "e" : "",
			ConfigChannel.use_forward ? "f" : "",
			ConfigChannel.use_halfop ? "h" : "",
			strcasecmp(ConfigChannel.disabledmodes, "j") ? "" : "j",
			strcasecmp(ConfigChannel.disabledmodes, "Z") ? "" : "Z",
 ...
 */
int
add_id(struct Client *source_p, struct Channel *chptr, const char *maskid,
       rb_dlink_list * list, long mode_type)
{
	...
	if(MyClient(source_p))
	{
		if(mode_type == CHFL_BAN || mode_type == CHFL_QUIET || mode_type == CHFL_EXCEPTION)
		{
			if((rb_dlink_list_length(&chptr->banlist) + rb_dlink_list_length(&chptr->exceptlist) + 
			    rb_dlink_list_length(&chptr->invexlist) + rb_dlink_list_length(&chptr->quietlist)) >= 
			    (chptr->mode.mode & MODE_EXLIMIT ? ConfigChannel.max_bans_large : ConfigChannel.max_bans))
			{
				sendto_one(source_p, form_str(ERR_BANLISTFULL),
					   me.name, source_p->name, chptr->chname, realmask);
				return 0;

			}

			/* XXX Oh yuck code duplication, got a better way? */
		...
		else if(mode_type == CHFL_CENSOR)
		{
			if((rb_dlink_list_length(&chptr->censorlist) >= 
				(chptr->mode.mode & MODE_EXLIMIT ? ConfigChannel.max_censors_large : ConfigChannel.max_censors)))
			{
				sendto_one(source_p, form_str(ERR_CENSORLISTFULL),
			...
			{
				actualModeItem = ptr->data;
				if(strcmp(actualModeItem->maskstr, realmask) == 0)
				{
					sendto_one(source_p, form_str(ERR_ALREADYONCENSORLIST),
						   me.name, source_p->name, chptr->chname);
					return 0;
				}
			}
	...
	actualModeItem = allocate_list_item(realmask, who);
 ...
 */
int
del_id(struct Client *source_p, struct Channel *chptr, const char *maskid, rb_dlink_list * list, long mode_type)
{
	rb_dlink_node *ptr;
	   ...
	   const char **parv, int *errors, int dir, char c, long mode_type)
{
	...
	if(alevel != CHFL_CHANOP && alevel != CHFL_ADMIN && alevel != CHFL_FOUNDER && alevel != CHFL_HALFOP)
	{
...
void
chm_list(struct Client *source_p, struct Channel *chptr,
	int alevel, int parc, int *parn,
	const char **parv, int *errors, int dir, char c, long mode_type)
{
	const char *mask;
	...
	case CHFL_CENSOR:
		list = &chptr->censorlist;
		errorval = SM_ERR_RPL_D;
		rpl_list = RPL_CENSORLIST;
		rpl_endlist = RPL_ENDOFCENSORLIST;
		mems = ALL_MEMBERS;
		caps = 0;
		break;

	default:
		sendto_realops_snomask(SNO_GENERAL, L_ALL, "chm_list() called with unknown type!");
		return;
		break;
	}
	...
	if(dir == 0 || parc <= *parn)
	{
		...
		*errors |= errorval;
		...
		if(alevel != CHFL_CHANOP && alevel != CHFL_ADMIN && alevel != CHFL_FOUNDER && 
				alevel != CHFL_HALFOP && mode_type != CHFL_BAN && mode_type != CHFL_QUIET)
		{
			if(!(*errors & SM_ERR_NOOPS))
				sendto_one(source_p, form_str(ERR_CHANOPRIVSNEEDED),
					   me.name, source_p->name, chptr->chname);
		...
		RB_DLINK_FOREACH(ptr, list->head)
		{
			...
			sendto_one(source_p, form_str(rpl_list),
				   me.name, source_p->name, chptr->chname,
				   listptr->maskstr, listptr->who, listptr->when);
		}
	...
	{
		if(strlen(mask) > IRCD_MIN(CENSORLEN, MODEBUFLEN - 5))
		{
			if(!(*errors & errorval))
				sendto_one(source_p, form_str(ERR_INVALIDCENSOR), me.name, source_p->name, chptr->chname);
			*errors |= errorval;
			return;
		}
	}

	/* If we're not +W, we're not allowed to even touch censor entries... 
	if(mode_type == CHFL_CENSOR && !(chptr->mode.mode & MODE_CENSORYES))
	{
		if(!(*errors & SM_ERR_RPL_D))
			sendto_one(source_p, form_str(ERR_CANTUSECENSOR), me.name, source_p->name, chptr->chname);
		*errors |= SM_ERR_RPL_D;
		return;
	}*/
	...
	{
		if(del_id(source_p, chptr, mask, list, mode_type) == 0)
		{
			/* mask isn't a valid ban, check raw_mask */
			if(del_id(source_p, chptr, raw_mask, list, mode_type))
				mask = raw_mask;
		}
       ...
       const char **parv, int *errors, int dir, char c, long mode_type)
{
	...
	{
		if(!(*errors & SM_ERR_NOOPS))
			sendto_one(source_p, ":%s 482 %s %s :You're not a channel administrator",
				   me.name, source_p->name, chptr->chname);
		*errors |= SM_ERR_NOOPS;
		return;
	}
	...
	const char **parv, int *errors, int dir, char c, long mode_type)
{
	...
	foundernick = parv[(*parn)];
	(*parn)++;

	/* empty nick */
	if(EmptyString(foundernick))
	{
		sendto_one_numeric(source_p, ERR_NOSUCHNICK, form_str(ERR_NOSUCHNICK), "*");
		return;
	}

	if((targ_p = find_chasing(source_p, foundernick, NULL)) == NULL)
	{
		return;
	}
	...
	if(mstptr == NULL)
	{
		if(!(*errors & SM_ERR_NOTONCHANNEL) && MyClient(source_p))
			sendto_one_numeric(source_p, ERR_USERNOTINCHANNEL,
					   form_str(ERR_USERNOTINCHANNEL), foundernick, chptr->chname);
		*errors |= SM_ERR_NOTONCHANNEL;
		return;
	}
	...
	if(dir == MODE_ADD)
	{
		if(targ_p == source_p && mstptr->flags & CHFL_FOUNDER)
			return;
...
struct ChannelMode chmode_table[256] =
{
  ...
  {chm_nosuch,	0 },					/* B */
  {chm_nosuch,	0 },					/* C */
  {chm_simple,	MODE_DISFORWARD },		/* D */
  {chm_staff,	MODE_EXLIMIT },		    /* E */
  {chm_simple,	MODE_FREETARGET },		/* F */
  {chm_nosuch,	0 },					/* G */
  {chm_nosuch,	0 },					/* H */
  {chm_list,	CHFL_INVEX },           /* I */
  {chm_simple,	MODE_NOREJOIN },		/* J */
  {chm_nosuch,  0 },                    /* K */
  {chm_forward,	0 },			        /* L */
  {chm_staff,	MODE_NOOPERKICK },		/* M */
  {chm_simple,	MODE_NONICK },			/* N */
  {chm_nosuch,	0 },					/* O */
  {chm_staff,	MODE_PERMANENT },		/* P */
  {chm_simple,	MODE_NOKICK },	    	/* Q */
  {chm_nosuch,	0 },					/* R */
  {chm_nosuch,	0 },					/* S */
  {chm_nosuch,	0 },					/* T */
  {chm_nosuch,	0 },					/* U */
  ...
  {chm_nosuch,	0 },
  {chm_nosuch,	0 },
  {chm_admin,	CHFL_ADMIN },			/* a */
  {chm_list,	CHFL_BAN },				/* b */
  {chm_nosuch,	0 },					/* c */
  {chm_nosuch,	0 },			        /* d */
  {chm_list,	CHFL_EXCEPTION },		/* e */
  {chm_nosuch,  0 },					/* f */
  {chm_list,	CHFL_CENSOR },			/* g */
  {chm_halfop,	0 },					/* h */
