...
stats_memory (struct Client *source_p)
{
	...
	size_t channel_censor_memory = 0;
	...
	RB_DLINK_FOREACH(ptr, global_channel_list.head)
	{
		...
		RB_DLINK_FOREACH(rb_dlink, chptr->censorlist.head)
		{
			actualModeItem = rb_dlink->data;
			channel_censors++;

			channel_censor_memory += (sizeof(rb_dlink_node) + sizeof(struct mode_list_t));
		}
	}
	...
	sendto_one_numeric(source_p, RPL_STATSDEBUG,
			   "z :Bans %u(%d) Exceptions %u(%d) Invex %u(%d) Quiets %u(%d) Censors %u(%d)",
			   ...
			   channel_censors, (int) channel_censor_memory);
