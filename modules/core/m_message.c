	    ...
	    const char *text)
{
	...
	if(result != CAN_SEND_NO && result != CAN_SEND_NO_NONOTIFY)
	{
		...
		/* XXX modularise me!!! */
        	if(ConfigChannel.use_censor &&
		   rb_dlink_list_length(&chptr->censorlist) != 0 &&
		   (!ConfigChannel.exempt_cmode_y || !is_any_op(msptr)))
		{
			char *text3 = LOCAL_COPY(text);

			strip_unprintable(text, text3, BUFSIZE);

			if(!EmptyString(text3))
			{
				if(match_censor(chptr, text3))
				{
...
match_censor(struct Channel *chptr, const char *text)
{
	...
	rb_dlink_list *list = &chptr->censorlist;
	rb_dlink_node *ptr;
