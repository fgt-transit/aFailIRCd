...
static int can_kick(struct membership *, struct membership *);
...
m_kick(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	*buf = '\0';
	...
	if(!IsServer(source_p))
	{
		...
		if(!can_kick(msptr, find_channel_membership(chptr, who)))
		{
 ...
 */
static int
can_kick(struct membership *source, struct membership *target)
{
	int sflag = get_channel_access(source->client_p, source);
